    ...
    def promedio_notas_criterios(self, notas, cantidad, criterios_controller):
        ...
        for i in self.evaluaciones:
            for j in range(len(notas)):
                if len( i.calificacion ) > 0 and len( notas ) <= i.calificacion and i.calificacion[j].identificador == criterios_controller.criterios[j].identificador: # revisa que solo se tenga en cuenta a personas que ya se calificaron
